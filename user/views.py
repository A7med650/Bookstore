...
def specialeducation(request):
    ...
    return render(request, "specialeducation.html")
...
def sign_up(request):
    ...
    if (
        ...
        EmailThread(email).start()
        return render(request, "emailSendConfirmation_signup.html")
...
def activate(request, uidb64, token):
    ...
    if (
        ...
        user.save()
        return redirect("/")
    return render(request, "youHaveUsedThisLinkBefore.html")
...
def forgetpass(request):
    ...
    if request.method == "POST":
        ...
        if not Check.valid_email_pass:
            ...
            EmailThread(email).start()
            return render(request, "emailSendConfirmation.html")
